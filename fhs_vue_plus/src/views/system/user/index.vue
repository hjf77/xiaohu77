<template>
  <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8" style="height:40px;">
        ...
        >
          ...
          <el-table-column
            ...
            width="120"
          />
          <el-table-column label="状态"  prop="isEnableName" align="center">
    ...
    <!-- 添加或修改参数配置对话框 -->
    <el-dialog class="dialog-theme" :title="title" :visible.sync="open" width="50%">
      <el-form class="pagex-from-theme" ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="12">
...
export default {
  name: "User",
  components: {Organization, Treeselect },
  data() {
    return {
      treeApi:"/ms/sysOrganization/tree",
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.open = true;
      this.title = "添加用户";
      this.form.newPassword = this.initPassword;
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getUser(userId).then((response) => {
        this.form = response;
        getSelectOrganSysRoles(response.organizationId).then((response) => {
          this.roleOptions = response;
        });
        var roleidArray = [];
        var roles = response.roleList;
        for (var item in roles) {
          var role = roles[item].replace("'", " ").replace("'", " ");
          roleidArray.push(Number(role));
        }
        this.form.roleIds = roleidArray;
        this.form.sex = response.sex.toString();
        this.form.isEnable = response.isEnable.toString();
        this.title = "修改用户";
        this.form.newPassword = "defaultPass";
      });
    },
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
    ...
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          let subFormData = JSON.parse(JSON.stringify(this.form))
          var roleIds = subFormData.roleIds.join(",");
          subFormData.roleIds = roleIds;
          subFormData.roleList = null;
          subFormData.password = subFormData.newPassword !='defaultPass' ? md5(subFormData.newPassword) : subFormData.newPassword ;
          if (subFormData.userId != undefined) {
            updateUser(subFormData).then((response) => {
              if (response.code === 200) {
            ...
            addUser(subFormData).then((response) => {
              if (response.code === 200) {
...
::v-deep .user-filter .el-form-item {
  margin-bottom:10px;
}
