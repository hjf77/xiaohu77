...
// 弹出加载层
function load() {
    $("<div class=\"datagrid-mask\"></div>").css({ display: "block", width: "100%", height: $(window).height(),'z-index':1000000}).appendTo("body");
    $("<div class=\"datagrid-mask-msg\"></div>").css({'z-index':1000001}).html("正在请求，请稍候。。。").appendTo("body").css({ display: "block", left: ($(document.body).outerWidth(true) - 190) / 2, top: ($(window).height() - 45) / 2 });
}
