...
public class FlowCoreServiceImpl implements FlowCoreService, FeignWorkFlowApiService {
    ...
    public List<BackAvtivityVO> findBackAvtivity(String taskId) throws Exception {
         ...
         */
        // 根据排序字段获取所有的历史记录
        List<FlowTaskHistoryVO> taskHistories = taskHistoryService.selectPageForOrder(FlowTaskHistoryDO.builder().instanceId(task.getProcessInstanceId())
    ...
    public void updateJointProcess(String taskId, List<String> userIds) throws Exception {
        ...
        for (String userId : userIds) {
            ...
            FlowTaskHistoryDO history = FlowTaskHistoryDO.builder().id(StringUtil.getUUID())
    ...
    public void commitProcess(String taskId, Map<String, Object> variables,
        ...
        Task task = this.findTaskById(taskId);
        FlowTaskHistoryVO history = this.taskHistoryService.selectBean(FlowTaskHistoryDO.builder().taskId(taskId).build());
        if (history != null) {
            ...
            taskHistoryService.update(history);
            //创建一个扔进去
        } else {
    ...
    private void checkProProcessInstanceIsEnd(String processInstanceId, String taskId) {
        ...
        if (processInstance == null) {
            FlowInstanceVO instance = this.flowInstanceService.selectBean(FlowInstanceDO.builder().activitiProcessInstanceId(processInstanceId).build());
            if (instance != null) {
                List<FlowTaskHistoryVO> histories = this.taskHistoryService.selectPageForOrder(FlowTaskHistoryDO.builder().instanceId(processInstanceId).build(), 0, 1, "create_time DESC");
