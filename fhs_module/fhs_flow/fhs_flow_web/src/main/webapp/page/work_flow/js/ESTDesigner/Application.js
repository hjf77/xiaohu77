...
ESTDesigner.tool.Parser = Class.extend({
	...
	_parseConnection : function() {
		...
		lines.each(function(i) {
					...
					var startPort = null;
					var endPort = null;
					var tempMinVal = 100000;
					connectionEdge.startX = parseInt(connectionEdge.startX);
			        connectionEdge.startY = parseInt(connectionEdge.startY);
					connectionEdge.endX = parseInt(connectionEdge.endX);
					connectionEdge.endY = parseInt(connectionEdge.endY);
					for (var i = 0; i < sPorts.getSize(); i++) {
						...
						var portSubVal =  Math.abs(connectionEdge.startX-x) + Math.abs(connectionEdge.startY-y);
						if(portSubVal<tempMinVal){
							startPort = s;
					...
					for (var i = 0; i < tPorts.getSize(); i++) {
						...
						var portSubVal =  Math.abs(connectionEdge.endY-x) + Math.abs(connectionEdge.endX-y);
						if(portSubVal<tempMinVal){
							endPort = t;
							tempMinVal = portSubVal;
						}
