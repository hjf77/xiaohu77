...
import { getFieIdList } from "./generate/formCreate/config/base/field.js";
import { getDictList } from "./generate/formCreate/utils/index.js";
    ...
    async initData() {
      ...
      getDictList().then((res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          const item = res[i];
          dictOptions.push({
            lable: item.groupName + "(" + item.groupCode + ")",
            value: item.groupCode,
          });
        }
        sessionStorage.setItem("fieIds", JSON.stringify(dictOptions));
      });
    },
