...
public class ExcelBaseUtils {
    ...
    public static OutputStream exportExcel(OutputStream outputStream, List<ExcelField> fields, List<?> datas) {
        try {
            ...
            for (int i = 0; i < fields.size(); i++) {
                ...
                sheet.setColumnWidth(i, excelField.getCellWidth()); //设置列宽
            ...
            for (int i = 0; i < datas.size(); i++) {
                ...
                for (int j = 0; j < fields.size(); j++) {
                    ...
                    sheet.setColumnWidth(j, excelField.getCellWidth()); //设置列宽
                    cell.setCellStyle(cellStyle);
     ...
     * @return
     */
    public static <T> List<T> easyImportExcel(InputStream is, List<ExcelField> fields, Class<T> resultClass, ServiceDictItemService dictItemService) {
        ...
        //校验
        StringBuilder validStr = valid(dataList, fields,  dictItemService);
     ...
     * @return
     */
    public static StringBuilder valid(List<Map<String, String>> dataList, List<ExcelField> fields, ServiceDictItemService dictItemService) {
        ...
        for (ExcelField field : fields) {
            ...
            if (!StringUtils.isBlank(field.getType()) && field.getType().contains("select")) {
                List<ServiceDictItemVO> wordBookList = dictItemService
                        .selectListMP(new LambdaQueryWrapper<ServiceDictItemPO>().eq(ServiceDictItemPO::getDictGroupCode,field.getDictCode()));
                List<String> wordbookDescList = wordBookList.stream().map(ServiceDictItemVO::getDictDesc).collect(Collectors.toList());
                if (!wordbookDescList.isEmpty()) {
                    wordbookMap.put(field.getDictCode(), wordbookDescList);
                }
            }
        }
        for (int i = 0; i < dataList.size(); i++) {
            ...
            for (String field : dataMap.keySet()) {
                ...
                if (!StringUtils.isBlank(excelField.getType()) && excelField.getType().contains("select")) {
                    //查询字典
                    if (wordbookMap.containsKey(excelField.getDictCode())) {
                        List<String> wordbookCodeList = wordbookMap.get(excelField.getDictCode());
     ...
     * @param fields
     */
    public static void exportTemplate(OutputStream outputStream, List<ExcelField> fields, ServiceDictItemService dictItemService) {
        SXSSFWorkbook workbook = new SXSSFWorkbook();
        SXSSFSheet sheet = workbook.createSheet();
        //设置边框颜色
        CellStyle cellStyle = PoiExcelUtils.setCellStyle(workbook);
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (!StringUtils.isBlank(excelField.getType()) && excelField.getType().contains("select")) {
                List<ServiceDictItemVO> wordBookList = dictItemService
                        .selectListMP(new LambdaQueryWrapper<ServiceDictItemPO>().eq(ServiceDictItemPO::getDictGroupCode,excelField.getDictCode()));
                List<String> wordbookDescList = wordBookList.stream().map(ServiceDictItemVO::getDictDesc).collect(Collectors.toList());
                if (!wordbookDescList.isEmpty()) {
            ...
            cell.setCellStyle(cellStyle);
            cell.setCellValue(excelField.getLabel());
        }
        try {
